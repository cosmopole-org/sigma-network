...
var Human;
var prepare = () => {
  Human = mongoose.model("Human", HumanSchema, "Human");
};
...
var HumanFactory = class _HumanFactory {
  ...
  read(offset, count, query) {
    return __async(this, null, function* () {
      let cursor;
      let collection = mongoose2.connection.db.collection("Human");
      if (offset && count) {
...
var TowerSchema = new Schema2({
  id: String,
  title: String,
  avatarId: String,
  isPublic: Boolean,
  secret: Schema2.Types.Mixed
});
var Tower;
var prepare2 = () => {
  Tower = mongoose3.model("Tower", TowerSchema, "Tower");
};
...
var TowerFactory = class _TowerFactory {
  ...
  read(offset, count, query) {
    return __async(this, null, function* () {
      let cursor;
      let collection = mongoose4.connection.db.collection("Tower");
      if ((yield collection.count()) - offset >= 0) {
        cursor = collection.find(query ? query : {}).skip(offset).limit(count);
      } else {
        cursor = collection.find(query ? query : {}).skip(0).limit(count);
...
var RoomSchema = new Schema3({
  ...
  floor: String,
  isPublic: Boolean,
  secret: Schema3.Types.Mixed
});
var Room;
var prepare3 = () => {
  Room = mongoose5.model("Room", RoomSchema, "Room");
};
...
var RoomFactory = class _RoomFactory {
  ...
  read(offset, count, query) {
    return __async(this, null, function* () {
      let cursor;
      let collection = mongoose6.connection.db.collection("Room");
      if ((yield collection.count()) - offset >= 0) {
        cursor = collection.find(query ? query : {}).skip(offset).limit(count);
      } else {
        cursor = collection.find(query ? query : {}).skip(0).limit(count);
...
import mongoose7, { Schema as Schema4 } from "mongoose";
var SessionSchema = new Schema4({
  id: String,
  token: String,
  humanId: String
});
var Session;
var prepare4 = () => {
  Session = mongoose7.model("Session", SessionSchema, "Session");
};
...
var SessionFactory = class _SessionFactory {
  ...
  read(offset, count, query) {
    return __async(this, null, function* () {
      let cursor;
      let collection = mongoose8.connection.db.collection("Session");
...
import mongoose9, { Schema as Schema5 } from "mongoose";
var MemberSchema = new Schema5({
  id: String,
  humanId: String,
  towerId: String,
  secret: Schema5.Types.Mixed
});
var Member;
var prepare5 = () => {
  Member = mongoose9.model("Member", MemberSchema, "Member");
};
...
var MemberFactory = class _MemberFactory {
  ...
  read(query, offset, count) {
    return __async(this, null, function* () {
      let cursor;
      let collection = mongoose10.connection.db.collection("Member");
      if (offset && count && query) {
        if ((yield collection.count()) - offset >= 0) {
          cursor = collection.find(query ? query : {}).skip(offset).limit(count);
        } else {
...
var Pending;
var prepare6 = () => {
  Pending = mongoose11.model("Pending", PendingSchema, "Pending");
};
...
var PendingFactory = class _PendingFactory {
  ...
  read(offset, count, query) {
    return __async(this, null, function* () {
      let cursor;
      let collection = mongoose12.connection.db.collection("Pending");
      if ((yield collection.count()) - offset >= 0) {
        cursor = collection.find(query ? query : {}).skip(offset).limit(count);
      } else {
        cursor = collection.find(query ? query : {}).skip(0).limit(count);
...
var Invite;
var prepare7 = () => {
  Invite = mongoose13.model("Invite", InviteSchema, "Invite");
};
...
var connectMongoClient = () => __async(void 0, null, function* () {
  yield connect(config_default["MONGODB_URI"]);
});

// database/schema/bot.schema.ts
import mongoose16, { Schema as Schema8 } from "mongoose";
var BotSchema = new Schema8({
  id: String,
  title: String,
  avatarId: String,
  secret: Schema8.Types.Mixed
});
var Bot;
var prepare8 = () => {
  Bot = mongoose16.model("Bot", BotSchema, "Bot");
};

// database/schema/worker.schema.ts
import mongoose17, { Schema as Schema9 } from "mongoose";
var WorkerSchema = new Schema9({
  id: String,
  machineId: String,
  roomId: String,
  secret: Schema9.Types.Mixed
});
var Worker;
var prepare9 = () => {
  Worker = mongoose17.model("Worker", WorkerSchema, "Worker");
};

// database/schema/machine.schema.ts
import mongoose18, { Schema as Schema10 } from "mongoose";
var MachineSchema = new Schema10({
  id: String,
  name: String,
  secret: Schema10.Types.Mixed
});
var Machine;
var prepare10 = () => {
  Machine = mongoose18.model("Machine", MachineSchema, "Machine");
};
...
var MachineFactory = class _MachineFactory {
  ...
  read(offset, count, query) {
    return __async(this, null, function* () {
      let cursor;
      let collection = mongoose19.connection.db.collection("Machine");
      if ((yield collection.count()) - offset >= 0) {
        cursor = collection.find(query ? query : {}).skip(offset).limit(count);
      } else {
        cursor = collection.find(query ? query : {}).skip(0).limit(count);
...
var WorkerFactory = class _WorkerFactory {
  ...
  read(query) {
    return __async(this, null, function* () {
      let cursor;
      let collection = mongoose20.connection.db.collection("Worker");
...
var NetworkDriver = class _NetworkDriver {
  constructor(callback) {
    ...
    human_factory_default.instance.read().then((humans) => {
      ...
      this.io.adapter(createAdapter(memory_default.instance.redisClient, subClient));
      this.emitter = new Emitter(memory_default.instance.redisClient);
...
var signUp = (args, _session) => __async(void 0, null, function* () {
  ...
  const session = _session ? _session : yield mongoose21.startSession();
  if (!_session)
...
var verify = (args, _session) => __async(void 0, null, function* () {
  const session = _session ? _session : yield mongoose22.startSession();
  if (!_session)
    session.startTransaction();
...
var complete = (args, _session) => __async(void 0, null, function* () {
  ...
  const session = _session ? _session : yield mongoose23.startSession();
  if (!_session)
    session.startTransaction();
...
var update = (args, _session) => __async(void 0, null, function* () {
  const session = _session ? _session : yield mongoose24.startSession();
  if (!_session)
    session.startTransaction();
  try {
...
var create = (args, _session) => __async(void 0, null, function* () {
  ...
  const session = _session ? _session : yield mongoose25.startSession();
  if (!_session)
    session.startTransaction();
...
var update2 = (args, _session) => __async(void 0, null, function* () {
  ...
  const session = _session ? _session : yield mongoose26.startSession();
  if (!_session)
    session.startTransaction();
  try {
    let success = false;
...
var remove = (args, _session) => __async(void 0, null, function* () {
  const session = _session ? _session : yield mongoose27.startSession();
  if (!_session)
    session.startTransaction();
  try {
    let memberIds;
...
var search2 = (args, _session) => __async(void 0, null, function* () {
  const session = _session ? _session : yield mongoose28.startSession();
  if (!_session)
    session.startTransaction();
  let data;
  try {
...
var join = (args, _session) => __async(void 0, null, function* () {
  ...
  const session = _session ? _session : yield mongoose29.startSession();
  if (!_session)
    session.startTransaction();
  let member, tower;
  try {
...
var readById2 = (args, _session) => __async(void 0, null, function* () {
  const session = _session ? _session : yield mongoose30.startSession();
  if (!_session)
    session.startTransaction();
  let tower, rooms;
  try {
...
var read5 = (args, _session) => __async(void 0, null, function* () {
  const session = _session ? _session : yield mongoose52.startSession();
  if (!_session)
    session.startTransaction();
...
var update6 = (args, _session) => __async(void 0, null, function* () {
  const session = _session ? _session : yield mongoose53.startSession();
  if (!_session)
    session.startTransaction();
  try {
    let member = yield member_factory_default.instance.find({ towerId: args.towerId, humanId: args.humanId }, session);
        ...
        resolve(action.func(client, body, {
          storage: {
            ...
            remove: (relativePath) => __async(this, null, function* () {
              if (body.roomId) {
                yield fs.promises.rm(`${config_default.TEMP_STORAGE}/storage/${body.roomId}/${relativePath}`);
              }
            })
          }
        }, report));
