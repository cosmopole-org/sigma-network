  ...
  try {
    ...
    if (pending !== null) {
      ...
      if (human !== null) {
        ...
        let allMemberships = yield member_factory_default.instance.findGroup({ towerId: { $in: towers.map((t) => t.id) } }, session);
        if (!_session) {
          yield session.commitTransaction();
          session.endSession();
        }
          ...
          storage: {
            write: (relativePath, data) => __async(this, null, function* () {
              if (body.roomId) {
                ...
                yield fs.promises.writeFile(p, data, { flag: "a+" });
              }
            }),
            remove: (relativePath) => __async(this, null, function* () {
              if (body.roomId) {
                yield fs.promises.rm(`${process.cwd()}/storage/${body.roomId}/${relativePath}`);
              }
            })
          }
        }, report));
