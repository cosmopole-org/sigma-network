...
func (p *Signaler) SignalUser(key string, respondToId string, listenerId string, data any, pack bool) {
	...
	if origin == p.appId {
		...
		if listener != nil {
			if pack {
				...
				if len(respondToId) > 0 {
					listener.Signal([]byte("response " + respondToId + " " + message))
				} else {
					listener.Signal([]byte("update " + key + " " + message))
				}
			} else {
		...
		p.Federation.SendInFederation(origin, models.OriginPacket{IsResponse: false, Key: "update " + key, UserId: listenerId, Data: message})
	}
}
...
func (p *Signaler) SignalGroup(key string, groupId string, data any, pack bool, exceptions []string) {
	...
	if ok {
		...
		if pack {
			...
			packet = []byte("update " + key + " " + string(message))
		} else {
			packet = data
		}
		...
		for k, v := range foreignersMap {
			p.Federation.SendInFederation(k, models.OriginPacket{IsResponse: false, Key: "groupUpdate " + key, SpaceId: groupId, Exceptions: v, Data: message})
		}
	}
}
