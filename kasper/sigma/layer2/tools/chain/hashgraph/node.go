...
type Node struct {
	sync.RWMutex
	execCb                        func(hashgraph_models.Request)
...
func NewNode(sigmaCore abstract.ICore, initialHashgraph map[string][]*Event, address string, executionCallback func(hashgraph_models.Request)) *Node {
	return &Node{
...
type Transaction struct {
	...
	Payload       string `json:"payload"` // amount
}
...
func (n *Node) FindOrder() {
	...
	addedEvents := n.ConsensusEvents[before:]

	trxs := []Transaction{}
	for _, e := range addedEvents {
		trxs = append(trxs, e.Transactions...)
	}
	for _, trx := range trxs {
		if trx.Type == "request" {
			packet := hashgraph_models.Request{}
			err := json.Unmarshal([]byte(trx.Payload), &packet)
			if err != nil {
				log.Println(err)
				continue
			}
