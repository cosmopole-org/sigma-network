...
func NewDLedgerFromPeers(sigmaCore abstract.ICore, port string, peerAddressMap map[string]string, execCb func(hashgraph_models.Request)) *DLedger {
	...
	// Setup the Hashgraph
	signature := crypto.SecureUniqueString()
	...
	myNode := hashgraph.NewNode(sigmaCore, initialHashgraph, myAddress, execCb)
...
func NewDLedger(sigmaCore abstract.ICore, port string, peersFilePath string, execCb func(hashgraph_models.Request)) *DLedger {
	localIPAddress := getLocalAddress()
	peerAddressMap := readPeerAddresses(peersFilePath, localIPAddress)
	return NewDLedgerFromPeers(sigmaCore, port, peerAddressMap, execCb)
}
...
func (dl *DLedger) PerformTransaction(typ string, payload any) {
