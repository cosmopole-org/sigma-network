...
import { mainDrawerOpen, setMainDrawerSwitcher } from "@/app/app/layout";
...
export default function HomeMain() {
    const mainDrawerState = useHookstate(mainDrawerOpen);
    const containerRef = useRef<HTMLDivElement>(null);
    const drawerRef = useRef<HTMLDivElement>(null);
    setMainDrawerSwitcher((open: boolean) => {
        mainDrawerOpen.set(open);
    ...
    return (
        <div className="w-full h-full fixed">
            ...
            <HomeBottomNav style={{ transform: `translate(0px, ${mainDrawerState.get({ noproxy: true }) ? '0%' : '150%'})` }} />
        </div>
    );
}
