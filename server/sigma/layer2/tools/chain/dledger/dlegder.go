...
func NewDLedgerFromPeers(sigmaCore abstract.ICore, port string, peerAddressMap map[string]string) *DLedger {
...
func NewDLedger(sigmaCore abstract.ICore, port string, peersFilePath string) *DLedger {
	localIPAddress := getLocalAddress()
	peerAddressMap := readPeerAddresses(peersFilePath, localIPAddress)
	return NewDLedgerFromPeers(sigmaCore, port, peerAddressMap)
}
...
func (dl *DLedger) PerformTransaction(changes []abstract.Change) {
