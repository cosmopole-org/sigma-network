package sigma_network_security
...
func AuthWithToken(app interfaces.IApp, token string) (int64, int32) {
	var ac = authCache[token]
	if ac != nil {
		return ac.UserId, ac.UserType
	}
	var auth = app.GetMemory().Get("auth::" + token)
	var userId int64 = 0
...
func Authenticate(app interfaces.IApp, packet types.WebPacket) (int64, string, string) {
...
func AuthorizeHuman(app interfaces.IApp, token string, humanId int64, packet types.WebPacket) Location {
	...
	var memberData = app.GetMemory().Get(fmt.Sprintf("member::%d::%d", tid, humanId))
	var cityData = app.GetMemory().Get(fmt.Sprintf("city::%d", rid))
	if memberData != "" {
...
func AuthorizeMachine(app interfaces.IApp, token string, machineId int64, packet types.WebPacket) Location {
	...
	var workerData = app.GetMemory().Get(fmt.Sprintf("worker::%d", wid))
	if workerData != "" {
		...
		var cityData = app.GetMemory().Get(fmt.Sprintf("city::%s", rid))
		if cityData == "" {
			return Location{TowerId: 0, RoomId: 0}
		}
...
func HandleLocation(app interfaces.IApp, token string, userId int64, userType string, packet types.WebPacket) Location {
