...
func AddMember(control *runtime.Control, input inputs_spaces.AddMemberInput, info models.Info) (any, error) {
...
func RemoveMember(control *runtime.Control, input inputs_spaces.RemoveMemberInput, info models.Info) (any, error) {
	admin := models.Admin{}
...
func Create(control *runtime.Control, input inputs_spaces.CreateInput, info models.Info) (any, error) {
...
func Update(control *runtime.Control, input inputs_spaces.UpdateInput, info models.Info) (any, error) {
	admin := models.Admin{}
...
func Delete(control *runtime.Control, input inputs_spaces.DeleteInput, info models.Info) (any, error) {
	admin := models.Admin{}
	err := control.Trx.Where("user_id=?", info.User.Id).Where("space_id=?", input.SpaceId).First(&admin).Error()
	if err != nil {
		return nil, err
...
func Get(control *runtime.Control, input inputs_spaces.GetInput, info models.Info) (any, error) {
	space := models.Space{Id: input.SpaceId}
	control.Trx.First(&space)
	if space.IsPublic {
...
func Join(control *runtime.Control, input inputs_spaces.JoinInput, info models.Info) (any, error) {
	space := models.Space{Id: input.SpaceId}
...
func Run(app *runtime.App) {
	...
	app.Services().AddAction(runtime.ExtractFunction(app, AddMember))
	app.Services().AddAction(runtime.ExtractFunction(app, RemoveMember))
	...
	app.Services().AddAction(runtime.ExtractFunction(app, Join))
}
