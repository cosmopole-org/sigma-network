...
func (ss *StorageService) UploadFile(app *runtime.App, tx *gorm.DB, input dtos_storage.UploadDto, info models.Info) (any, error) {
	if !((len(input.Data) == 1) && (len(input.DataKey) == 1)) {
		return outputs_storage.Binary{}, errors.New("we need 1 file and 1 file key")
	}
	if err := ss.managers.StorageManager().SaveFileToStorage(app.StorageRoot, input.Data[0], info.Member.TopicIds, input.DataKey[0]); err != nil {
		log.Println(err)
...
func (ss *StorageService) DownloadFile(app *runtime.App, tx *gorm.DB, input dtos_storage.DownloadDto, info models.Info) (any, error) {
	var path = fmt.Sprintf("%s/%s/%s", app.StorageRoot, info.Member.TopicIds, input.FileKey)
